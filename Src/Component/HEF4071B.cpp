...
nts::HEF4071B::HEF4071B(const std::string name, nts::TypeOfComponent type)
...
nts::HEF4071B::HEF4071B(HEF4071B &other) : AbstractComponent(other._name,
...
nts::HEF4071B &nts::HEF4071B::operator=(HEF4071B &other)
{
	for (int i = 0; i < 14; ++i) {
		this->_pin[i] = nullptr;
		this->_pinLink.insert({i + 1, 0});
...
nts::IComponent *nts::HEF4071B::Copy()
{
	return new HEF4071B(*this);
}

void nts::HEF4071B::setLink(std::size_t pin, nts::IComponent &other,
	std::size_t otherPin
)
{
	if (pin - 1 < 14 && this->_pin[pin - 1] &&
		...
		try {
			...
			throw SetLinkError("SetLinkError: Can't link this pin",
				this->getName());
		}
	} else
...
void nts::HEF4071B::DefineLink(std::size_t pin, nts::IComponent &other,
	std::size_t otherPin
)
{
	if (pin - 1 < 14 && this->_pin[pin - 1] &&
...
nts::Tristate nts::HEF4071B::compute(std::size_t pin __attribute__((unused)))
{
	throw ComputeError("ComputeError: Can't compute this component",
		this->getName());
}

nts::IComponent *nts::HEF4071B::getPin()
{
	return *this->_pin;
}

std::unordered_map <std::size_t, std::size_t> nts::HEF4071B::getPinLink() const
{
	return this->_pinLink;
}

void nts::HEF4071B::resetSimulation()
{
	for (int i = 0; i < 14; ++i) {
