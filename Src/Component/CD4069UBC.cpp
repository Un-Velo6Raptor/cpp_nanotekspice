...
nts::CD4069UBC::CD4069UBC(const std::string name, nts::TypeOfComponent type)
	: AbstractComponent(name, type, nts::CHIPSET)
{
	...
	this->_pin[1] = new NotGate(name + ": Gate 2 (1)", this->_pin[0]);

	this->_pin[2] = new Input(name + ": Pin 3", nts::PIN);
	this->_pin[3] = new NotGate(name + ": Gate 4 (3)", this->_pin[2]);

	this->_pin[4] = new Input(name + ": Pin 5", nts::PIN);
	this->_pin[5] = new NotGate(name + ": Gate 6 (5)", this->_pin[4]);

	this->_pin[12] = new Input(name + ": Pin 13", nts::PIN);
	this->_pin[11] = new NotGate(name + ": Gate 12 (13)", this->_pin[12]);

	this->_pin[10] = new Input(name + ": Pin 11", nts::PIN);
	this->_pin[9] = new NotGate(name + ": Gate 10 (11)", this->_pin[10]);

	this->_pin[8] = new Input(name + ": Pin 9", nts::PIN);
	this->_pin[7] = new NotGate(name + ": Gate 8 (9)", this->_pin[8]);
}
...
nts::CD4069UBC::CD4069UBC(CD4069UBC &other) : AbstractComponent(other._name,
...
nts::CD4069UBC &nts::CD4069UBC::operator=(CD4069UBC &other)
{
	for (int i = 0; i < 14; ++i) {
		this->_pin[i] = nullptr;
		this->_pinLink.insert({i + 1, 0});
...
nts::IComponent *nts::CD4069UBC::Copy()
{
	return new CD4069UBC(*this);
}

void nts::CD4069UBC::setLink(std::size_t pin, nts::IComponent &other,
	std::size_t otherPin
)
{
...
void nts::CD4069UBC::DefineLink(std::size_t pin, nts::IComponent &other,
	std::size_t otherPin
)
{
	if (pin - 1 < 14 && this->_pin[pin - 1] &&
...
nts::Tristate nts::CD4069UBC::compute(std::size_t pin __attribute__((unused)))
{
	throw ComputeError("ComputeError: Can't compute this component",
		this->getName());
}

nts::IComponent *nts::CD4069UBC::getPin()
{
	return *this->_pin;
}

std::unordered_map <std::size_t, std::size_t> nts::CD4069UBC::getPinLink() const
{
	return this->_pinLink;
}

void nts::CD4069UBC::resetSimulation()
{
	for (int i = 0; i < 14; ++i) {
